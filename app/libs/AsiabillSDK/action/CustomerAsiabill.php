...
class CustomerAsiabill extends AsiabillClient
{
    ...
    {
        return $this->setRequestType('customers')->setBody(['body'=>$builder->toArray()])->send();
    }

    public function create_customer_payment_id(string $customer_id,PaymentMethodsBuilder $builder)
    {

        return $this->setRequestType('paymentMethods')->setBody(['body'=>$builder->setCustomerId($customer_id)->toArray()])->send();
    }
}